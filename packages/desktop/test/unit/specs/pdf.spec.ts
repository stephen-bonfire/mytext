...
// `window.mytext.paths` / `window.fileUtils` at call time, all normally
...
vi.hoisted(() => {
  const w = globalThis as unknown as {
    window?: {
      ...
      mytext?: { paths: { userDataPath: string } }
  ...
  w.window.mytext ??= { paths: { userDataPath: '/userData' } }
...
// never touch `window.fileUtils`/`window.mytext`, unlike a disk theme name)
...
describe('getCssForOptions', () => {
  ...
  it('appends no theme CSS for theme:"default" (disk lookup misses) or {}', async() => {
    ...
    // reads window.mytext.paths + window.fileUtils.isFile (→ false here).
