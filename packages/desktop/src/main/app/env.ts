...
const setupEnvironment = (args: Record<string, unknown>): AppEnvironment => {
  ...
  const debug =
    !!args['--debug'] || !!process.env.MYTEXT_DEBUG || process.env.NODE_ENV !== 'production'
  ...
  const mutableGlobal = global as unknown as {
    MYTEXT_DEBUG: boolean
    MYTEXT_DEBUG_VERBOSE: number
    MYTEXT_SAFE_MODE: boolean
  }
  mutableGlobal.MYTEXT_DEBUG = debug
  mutableGlobal.MYTEXT_DEBUG_VERBOSE = verbose
  mutableGlobal.MYTEXT_SAFE_MODE = safeMode
