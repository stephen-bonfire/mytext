...
class Keybindings {
  ...
  _prepareKeyMapper(): void {
    ...
    keyboardLayoutMonitor.addListener(({ layout, keymap }: KeyboardInfo) => {
      const globalDebug = (globalThis as typeof globalThis & { MYTEXT_DEBUG?: boolean })
        .MYTEXT_DEBUG
      if (globalDebug && process.env.MYTEXT_DEBUG_KEYBOARD) {
  ...
  _loadLocalKeybindings(): void {
    const safeMode = (globalThis as typeof globalThis & { MYTEXT_SAFE_MODE?: boolean })
      .MYTEXT_SAFE_MODE
