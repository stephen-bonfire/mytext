...
const cli = (): ParsedArgs => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to MyText and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'mytext-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: mytext [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`MyText: ${MYTEXT_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'mytext-user-data')
